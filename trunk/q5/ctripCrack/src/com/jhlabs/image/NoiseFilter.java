...
public class NoiseFilter extends PointFilter {
	...
	private float density = (float)18;
	...
	public int filterRGB(int x, int y, int rgb) {
		if (randomNumbers.nextFloat() <= density) {
			...
			int b = rgb & 0xff;
			if (monochrome) {
				int n = (int) (((distribution == GAUSSIAN ? randomNumbers
						.nextGaussian() : 2 * randomNumbers.nextFloat() - 1)) * amount);
	...
	public static void main(String[] args) throws IOException{
		File _file = new File("d:/11.jpg");
		Image src = javax.imageio.ImageIO.read(_file);
		int wideth=src.getWidth(null);
		int height=src.getHeight(null);
		BufferedImage sourceBuffer = new BufferedImage(wideth,height,BufferedImage.TYPE_INT_RGB);
		sourceBuffer.getGraphics().drawImage(src,0,0,wideth,height,null);
		BufferedImage destBuffer = new BufferedImage(wideth,height,BufferedImage.TYPE_INT_RGB);
		ReduceNoiseFilter filter=new ReduceNoiseFilter();
		filter.filter(sourceBuffer, destBuffer);
		FileOutputStream out=new FileOutputStream("d:/11-new.jpg");
		JPEGImageEncoder encoder = JPEGCodec.createJPEGEncoder(out);
		encoder.encode(destBuffer);
