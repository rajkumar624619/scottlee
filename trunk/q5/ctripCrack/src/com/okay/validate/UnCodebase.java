...
//		}
		...
		ImageData[] d = simpleSplit("D:\\2.jpg");
		for(ImageData id:d ){
			id.show();
		}
	}

	public static ImageData[] simpleSplit(String fileName) throws IOException, FileNotFoundException {
...
//		System.out.println(grayValue);
		...
		ImageData[] _imageSubs = imageData.simpleSplit(6);
		return _imageSubs;
	}
	
	
	public static ImageData[] split(String fileName, int column, boolean ifModify) throws IOException, FileNotFoundException {
...
//		System.out.println(grayValue);
		...
		imageData.modify();
		 ...
		 ImageData[] _imageSubs = imageData.split(column);
		 for(int i=0;i<_imageSubs.length;i++){
			 _imageSubs[i].show();
//			 _imageSubs[i].writeImg(new File("imgs/ctrip_"+f.getName().replace(".jpg", "")+"_"+index+"_"+i+".jpg"));
//			 _imageSubs[i].writeData(new File("data/ctrip_"+f.getName().replace(".jpg", "")+"_"+i+".txt"));
			 
		 }
		 return _imageSubs;
	}
	
	public static ImageData[] split(String fileName, int column) throws IOException, FileNotFoundException {
//		String fileName="D:\\dev\\cygwin\\home\\Scott.Lee\\ctrip_img";
		File f = new File(fileName);
		BufferedImage bi = ImageIO.read(f);
		UnCodebase uc = new UnCodebase(bi);
