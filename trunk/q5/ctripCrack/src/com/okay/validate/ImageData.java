...
public class ImageData {
	...
	public int[] getSplitPointsY() {
		...
		try {
			...
			Arrays.sort(length);
			minWidth = length[str.length - 1 ]-1;
			int tempx = 0;
			for (int i = 0; i < str.length; i++) {
				if (str[i].length() > minWidth) {
		...
		System.out.println(posx[0] + " " + posx[1]);
		if (posx[1] == 0 || minWidth<20) {
			posx[0] = 0;
			posx[1] = h;
		}
		System.out.println(posx[0] + " " + posx[1]);
	...
	public ImageData[] simpleSplit(int charNum) {
		ImageData[] sub = new ImageData[charNum];

		int[] splitPoints = getSplitPointsX2(charNum);
		...
		for (int i = 0; i < charNum; i++) {
			...
			int posyy[] = subImg.getSplitPointsY();
			posy = posyy[0];
			posy1 = posyy[1];
			System.out.println(posx + "|" + posy + "|" + posx1 + "|" + posy1
					+ "|" + lbound + "|" + ubound);
	...
	public void writeImg(File imgName, int border) throws IOException {
		try {
			BufferedImage img = new BufferedImage(this.w+2*border, this.h+2*border,
					BufferedImage.TYPE_INT_RGB);

			for (int i = 0; i < w+2*border; i++) {
				for (int j = 0; j < h+2*border; j++) {
					if(i<=border||i>=w+border||j<=border||j>=h+border){
						img.setRGB(i, j, 0xffffff);
					}else{
						int rgb = 0;
						if (data[j-border][i-border] == 0) {
							rgb = 0;
						} else {
							rgb = 0xffffff;
						}
