...
public class NoiseRemove {
	...
	public static void reserveSingleColor(BufferedImage binaryImage, BufferedImage rgbImage) {
		int height = binaryImage.getHeight(), width = binaryImage.getWidth();
		...
		ColorFilter filter = new ColorFilter(pixels, rgbImage);
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				if (!filter.isNormal(x, y)) {
					binaryImage.setRGB(x, y, -1);
				}
			}
		}
	...
	public static BufferedImage removeNoise(BufferedImage rgbImage) {
		...
		reserveSingleColor(binaryImage, rgbImage);
		deSpeckle(binaryImage);
